...
#define foreach(type, name, container) for (typename hltypes::Array< type >::iterator_t name = (container).begin(), name ## End = (container).end(); name != name ## End; ++name)
/// @brief Provides a simpler syntax to iterate through an Array.
#define foreachc(type, name, container) for (typename hltypes::Array< type >::const_iterator_t name = (container).begin(), name ## End = (container).end(); name != name ## End; ++name)
/// @brief Provides a simpler syntax to reverse iterate through an Array.
#define foreach_r(type, name, container) for (typename hltypes::Array< type >::riterator_t name = (container).rbegin(), name ## End = (container).rend(); name != name ## End; ++name)
/// @brief Provides a simpler syntax to reverse iterate through an Array.
#define foreachc_r(type, name, container) for (typename hltypes::Array< type >::const_riterator_t name = (container).rbegin(), name ## End = (container).rend(); name != name ## End; ++name)
...
namespace hltypes
{
	...
	public:
		...
		{
			return this->template _indices<Array<int> >();
		}
		/// @brief Gets all indices of the given element.
		/// @param[in] element Element to search for.
		/// @return All indices of the given element.
		inline Array<int> indicesOf(const T& element) const
		{
			return this->template _indicesOf<Array<int> >(element);
		}
